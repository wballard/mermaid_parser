...
use mermaid_parser::{
    parse_diagram, DiagramType, AstVisitor, NodeCounter, ComplexityAnalyzer, 
    ReferenceValidator, DiagramMetrics
};
...
"#;
    ...
    match parse_diagram(complex_flowchart)? {
        DiagramType::Flowchart(flowchart) => {
            ...
            let terminal_nodes: Vec<_> = flowchart.nodes.keys()
            ...
            let entry_nodes: Vec<_> = flowchart.nodes.keys()
    ...
    match parse_diagram(sankey_example)? {
        DiagramType::Sankey(sankey) => {
            ...
            let max_flow = sankey.links.iter().map(|link| link.value).fold(0.0, f64::max);
            let min_flow = sankey.links.iter().map(|link| link.value).fold(f64::INFINITY, f64::min);
            ...
            for node in &sankey.nodes {
                ...
                println!("    {}: in={:.1}, out={:.1}, balance={:.1}", 
                    node.name, incoming, outgoing, balance);
            }
            
            // Find source and sink nodes
            let sources: Vec<_> = sankey.nodes.iter()
                .filter(|node| !incoming_flow.contains_key(&node.id))
                .map(|node| &node.name)
                .collect();
            
            let sinks: Vec<_> = sankey.nodes.iter()
    ...
    for (name, diagram_source) in test_diagrams {
        ...
        match parse_diagram(diagram_source) {
            Ok(diagram) => {
                ...
                println!("    Cognitive Complexity: {:.2}", metrics.complexity.cognitive);
                println!("    Nesting Depth: {}", metrics.complexity.nesting_depth);
                println!("    Coupling: {:.2}", metrics.complexity.coupling);
                
                println!("  Quality Metrics:");
                println!("    Maintainability: {:.2}", metrics.quality.maintainability);
