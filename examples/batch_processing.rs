...
use mermaid_parser::{parse_diagram, DiagramType, DiagramMetrics};
...
impl BatchStats {
    ...
    fn add_success(&mut self, diagram: &DiagramType) {
        ...
        let (type_name, nodes, edges) = match diagram {
            ...
            DiagramType::Requirement(d) => ("Requirement", d.requirements.len(), d.relationships.len()),
    ...
    fn print_report(&self) {
        ...
        if self.successful_parses > 0 {
            println!("  Average nodes per diagram: {:.1}", 
                self.total_nodes as f64 / self.successful_parses as f64);
            println!("  Average edges per diagram: {:.1}", 
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_diagrams = vec![
        ("flowchart_simple.mmd", r#"
flowchart TD
    A[Start] --> B[Process]
    B --> C[End]
"#),
        ("flowchart_complex.mmd", r#"
flowchart LR
    subgraph Frontend
        A[User Interface]
        B[Form Validation]
...
"#),
    ...
    for (filename, content) in &test_diagrams {
        ...
        match parse_diagram(content) {
            Ok(diagram) => {
                ...
                println!("    Nodes: {}, Edges: {}, Complexity: {}", 
    ...
    println!("Processing {} diagrams in simulated parallel batch...", large_batch.len());
    ...
    println!("  Successful: {}/{}", parallel_stats.successful_parses, parallel_stats.total_files);
    println!("  Success rate: {:.1}%", 
