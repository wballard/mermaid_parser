...
pub fn detect_diagram_type(line: &str) -> Option<&'static str> {
    ...
    if diagram_headers::SEQUENCE_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("sequence");
    }
    
    if diagram_headers::STATE_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("state");
    }
    
    if diagram_headers::SANKEY_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("sankey");
    }
    
    if diagram_headers::TIMELINE_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("timeline");
    }
    
    if diagram_headers::GANTT_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("gantt");
    }
    
    if diagram_headers::PIE_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("pie");
    }
    
    if diagram_headers::C4_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("c4");
    }
    
    if diagram_headers::BLOCK_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("block");
    }
    
    if diagram_headers::PACKET_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("packet");
    }
    
    if diagram_headers::XYCHART_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("xychart");
    }
    
    if diagram_headers::TREEMAP_HEADERS.iter().any(|h| trimmed.starts_with(h)) {
        return Some("treemap");
    }
    ...
    if trimmed.starts_with(diagram_headers::QUADRANT) || trimmed.starts_with(diagram_headers::QUADRANT_CHART) {
    ...
    if trimmed.starts_with(diagram_headers::REQUIREMENT) || trimmed.starts_with(diagram_headers::REQUIREMENT_DIAGRAM) {
