...
use crate::common::ast::{XyChartDiagram, ChartOrientation, XAxis, YAxis, DataSeries, SeriesType, AccessibilityInfo};
...
fn parse_data_array(content: &str) -> Result<Vec<f64>> {
    ...
    for item in inner.split(',') {
        ...
        if !trimmed.is_empty() {
            let value = trimmed.parse::<f64>().map_err(|_| ParseError::SyntaxError {
...
fn parse_range(content: &str) -> Result<Option<(f64, f64)>> {
    if let Some(arrow_pos) = content.find("-->") {
        ...
        let start = start_str.parse::<f64>().map_err(|_| ParseError::SyntaxError {
        ...
        let end = end_str.parse::<f64>().map_err(|_| ParseError::SyntaxError {
