...
use crate::common::ast::{
    RequirementDiagram, Requirement, RequirementType, RiskLevel, VerificationMethod,
    Element, RequirementRelationship, RelationshipType, AccessibilityInfo,
};
...
pub fn requirement_lexer<'src>() -> impl Parser<'src, &'src str, Vec<RequirementToken>, extra::Err<Simple<'src, char>>> {
    ...
    let requirement_types = choice((
        ...
        text::keyword("physicalRequirement")
            .map(|_| RequirementToken::PhysicalRequirement),
    ...
    let number = text::int(10)
        .map(|s: &str| RequirementToken::Identifier(s.to_string()));
    ...
    let identifier = choice((
        ...
        just('_').repeated().at_least(1).collect::<String>()
...
fn requirement_parser<'src>() -> impl Parser<'src, &'src [RequirementToken], RequirementDiagram, extra::Err<Simple<'src, RequirementToken>>> {
    // Parse requirementDiagram header (skip leading newlines)
    // Accept both "requirementDiagram" and "requirement" as headers
    let header = any().filter(|t| matches!(t, RequirementToken::NewLine))
        .repeated()
        .ignore_then(choice((
        ...
        .then_ignore(
            any().filter(|t| matches!(t, RequirementToken::NewLine))
                .repeated()
        );
    
    // Parse requirement type tokens
    let requirement_type_token = any().try_map(|t, span| {
        match t {
    ...
    let identifier = any().try_map(|t, span| {
        match t {
    ...
    // Parse quoted string
    let _quoted_string = any::<&[RequirementToken], extra::Err<Simple<RequirementToken>>>().try_map(|t, span| {
        match t {
    ...
    let requirement_def = requirement_type_token
        ...
        .then(
            ...
            any().filter(|t| !matches!(t, RequirementToken::RightBrace))
                .repeated()
                .collect::<Vec<_>>()
        )
            ...
            while i < tokens.len() {
                match &tokens[i] {
                    RequirementToken::Id => {
                        ...
                        if i < tokens.len() && matches!(&tokens[i], RequirementToken::Identifier(s) if s == ":") {
                            i += 1;
                        }
                        if i < tokens.len() {
                    ...
                    RequirementToken::Text => {
                        ...
                        while i < tokens.len() {
                            match &tokens[i] {
                                ...
                                RequirementToken::Id | RequirementToken::VerifyMethod | RequirementToken::DocRef => {
                                    break;
                                }
                                _ => {
                        ...
                        text = text_parts.into_iter().fold(String::new(), |mut acc, part| {
                            if acc.is_empty() {
                                acc.push_str(&part);
                            } else if part.starts_with('.') || part.starts_with(',') || part.starts_with('!') || part.starts_with('?') {
            ...
            while i < tokens.len() {
                match &tokens[i] {
                    RequirementToken::Type => {
                        ...
                        element_type = type_parts.into_iter().fold(String::new(), |mut acc, part| {
                            if acc.is_empty() {
                                acc.push_str(&part);
                            } else if part.starts_with('.') || part.starts_with(',') || part.starts_with('!') || part.starts_with('?') {
                                acc.push_str(&part); // No space before punctuation
    ...
    // Parse relationship
    let relationship_def = identifier.clone()
        .then(
            choice((
                just(RequirementToken::Dash)
                    .ignore_then(any().try_map(|t, span| {
                        match t {
    ...
    let acc_descr_def = just(RequirementToken::AccDescr)
        .ignore_then(just(RequirementToken::Identifier(":".to_string())).or_not())
        .then(
            choice((
    ...
    let direction_def = just(RequirementToken::Direction)
        .then(
            any().filter(|t| !matches!(t, RequirementToken::NewLine))
                .repeated()
        );
    ...
    let style_def = just(RequirementToken::Style)
        .then(
            any().filter(|t| !matches!(t, RequirementToken::NewLine))
                .repeated()
        );
    ...
    let class_def_directive = just(RequirementToken::ClassDef)
        .then(
            any().filter(|t| !matches!(t, RequirementToken::NewLine))
                .repeated()
        );
    ...
    let class_directive = just(RequirementToken::Class)
        .then(
            any().filter(|t| !matches!(t, RequirementToken::NewLine))
                .repeated()
        );
    ...
    header
        .then(
            choice((
                ...
                acc_title_def.map(|(key, val)| ("".to_string(), None, None, None, Some((key, val)))),
                acc_descr_def.map(|(key, val)| ("".to_string(), None, None, None, Some((key, val)))),
                ...
                any().filter(|t| matches!(t, RequirementToken::NewLine | RequirementToken::Comment(_)))
