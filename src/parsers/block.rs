...
use crate::common::ast::{
    Block, BlockArrowType, BlockConnection, BlockDiagram, BlockShape, AccessibilityInfo,
};
...
fn block_lexer<'src>() -> impl Parser<'src, &'src str, Vec<BlockToken>, extra::Err<Simple<'src, char>>> {
    let comment = choice((
        just("%%").then(none_of('\n').repeated()),
        just("//").then(none_of('\n').repeated()),
    ))
    ...
    let identifier = text::ident()
        .map(|s: &str| BlockToken::BlockId(s.to_string()));
    ...
    let token = choice((
        ...
        space,
        arrows,
        block_rounded.map(|(id, label, shape)| BlockToken::BlockLabel(format!("{}:{}:{:?}", id, label, shape))),
        block_circle.map(|(id, label, shape)| BlockToken::BlockLabel(format!("{}:{}:{:?}", id, label, shape))),
        identifier,
    ))
    .padded();
...
fn block_parser<'tokens, 'src: 'tokens>() -> impl Parser<
    ...
    &'tokens [BlockToken],
    BlockDiagram,
    extra::Err<Simple<'tokens, BlockToken>>,
> + Clone {
        ...
        while i < tokens.len() {
            match &tokens[i] {
                ...
                BlockToken::BlockId(id) => {
                    ...
                    if i + 2 < tokens.len() {
                        if let (BlockToken::Arrow | BlockToken::DottedArrow | BlockToken::ThickArrow | BlockToken::InvisibleArrow | BlockToken::BiArrow, BlockToken::BlockId(to)) = (&tokens[i + 1], &tokens[i + 2]) {
...
"#;
        ...
        let space_count = diagram.blocks.iter()
