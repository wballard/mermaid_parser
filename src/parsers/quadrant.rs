use crate::common::ast::{QuadrantDiagram, AxisDefinition, QuadrantLabels, DataPoint, ClassDefinition, AccessibilityInfo};
...
fn parse_axis_definition(text: &str) -> Option<AxisDefinition> {
    if let Some(arrow_pos) = text.find("-->") {
        ...
        Some(AxisDefinition {
            label_start: if start_label.is_empty() { None } else { Some(start_label.to_string()) },
            label_end: if end_label.is_empty() { None } else { Some(end_label.to_string()) },
        })
    } else {
        None
    }
...
fn parse_data_point(name: &str, coords: &str, _line_num: usize) -> Option<DataPoint> {
    ...
    let (point_name, class_name) = if let Some(class_pos) = name.find(":::") {
        ...
        (name_part, if class_part.is_empty() { None } else { Some(class_part.to_string()) })
    } else {
        (name, None)
    };
