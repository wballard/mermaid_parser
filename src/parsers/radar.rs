...
use crate::common::ast::{RadarDiagram, RadarConfig, Dataset, AccessibilityInfo};
...
fn parse_config_line(line: &str, config: &mut RadarConfig) {
    ...
    if let Some(content) = line.strip_prefix("%%{init:").and_then(|s| s.strip_suffix("}%%")) {
