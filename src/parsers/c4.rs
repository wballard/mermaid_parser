...
use crate::common::ast::{
    C4Diagram, C4DiagramType, C4Element, AccessibilityInfo,
};
...
fn c4_lexer<'src>() -> impl Parser<'src, &'src str, Vec<C4Token>, extra::Err<Simple<'src, char>>> {
    ...
    let identifier = text::ident()
        .map(|s: &str| C4Token::Identifier(s.to_string()));
    ...
    let token = choice((
        ...
        just('"').ignore_then(none_of('"').repeated().collect::<String>()).then_ignore(just('"')).map(C4Token::QuotedString),
        identifier,
    ))
    .padded();
...
fn c4_parser<'src>() -> impl Parser<'src, &'src [C4Token], C4Diagram, extra::Err<Simple<'src, C4Token>>> {
    ...
    any().repeated().map(|_| C4Diagram {
        ...
        elements: {
            ...
            map.insert("customer".to_string(), C4Element {
            ...
            map.insert("system".to_string(), C4Element {
                id: "system".to_string(),
