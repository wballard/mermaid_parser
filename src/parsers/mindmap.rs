...
use crate::common::ast::{MindmapDiagram, MindmapNode, MindmapNodeShape, AccessibilityInfo};
...
fn build_children(parsed_lines: &[(usize, ParsedLine)], start_index: usize, parent_indent: usize) -> Vec<MindmapNode> {
    ...
    while i < parsed_lines.len() {
        ...
        if i > start_index {
            let prev_child_indent = parsed_lines.iter()
                .skip(start_index)
                .take(i - start_index)
                .filter_map(|(ind, _)| if *ind > parent_indent { Some(*ind) } else { None })
                .min()
