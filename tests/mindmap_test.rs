...
fn test_node_shapes() {
    ...
    for (input, expected_shape) in shapes_and_inputs {
        ...
        if diagram.root.children.is_empty() {
            ...
            assert_eq!(diagram.root.shape, expected_shape, "Shape mismatch for {}", input);
        } else {
            // Child node case
            assert_eq!(diagram.root.children[0].shape, expected_shape, "Shape mismatch for {}", input);
        }
    }
}
...
"#;
    ...
    let result = mindmap::parse(input);
    assert!(result.is_ok(), "Should parse mindmap with icons and classes");
