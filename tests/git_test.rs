...
fn test_git_files(#[files("test/git/*.mermaid")] path: PathBuf) {
    ...
    assert!(
        diagram.operations.len() > 0 || 
        diagram.title.is_some() ||
        diagram.branches.len() > 0,
        "Git graph {:?} should have operations, title, or branches", path
    );
}
...
"#;
    ...
    assert!(result.is_ok(), "Failed to parse simple git graph: {:?}", result);
...
"#;
    ...
    let result = git::parse(input);
    assert!(result.is_ok(), "Failed to parse git graph with colon: {:?}", result);
...
"#;
    ...
    let result = git::parse(input);
    assert!(result.is_ok(), "Failed to parse branch operations: {:?}", result);
    
    let diagram = result.unwrap();
    
    // Check for specific operation types
    let has_branch = diagram.operations.iter().any(|op| 
        matches!(op, mermaid_parser::common::ast::GitOperation::Branch { .. }));
    let has_merge = diagram.operations.iter().any(|op| 
        matches!(op, mermaid_parser::common::ast::GitOperation::Merge { .. }));
...
"#;
    ...
    let has_cherry_pick = diagram.operations.iter().any(|op| 
        matches!(op, mermaid_parser::common::ast::GitOperation::CherryPick { .. }));
...
"#;
    ...
    let result = git::parse(input);
    assert!(result.is_ok(), "Failed to parse accessibility: {:?}", result);
    
    let diagram = result.unwrap();
    assert_eq!(diagram.accessibility.title, Some("Git Graph".to_string()));
    assert_eq!(diagram.accessibility.description, Some("This is a git workflow diagram".to_string()));
}
...
"#;
    ...
    let result = git::parse(input);
    assert!(result.is_ok(), "Failed to parse title and theme: {:?}", result);
...
"#;
    ...
    let result = git::parse(input);
    assert!(result.is_ok(), "Failed to parse minimal git graph: {:?}", result);
...
fn test_empty_git_graph() {
    ...
    let result = git::parse(input);
    assert!(result.is_ok(), "Failed to parse empty git graph: {:?}", result);
