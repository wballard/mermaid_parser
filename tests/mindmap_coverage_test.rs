...
fn test_non_mindmap_header_error() {
    ...
    assert!(result.is_err());
    match result {
        Err(ParseError::SyntaxError { message, expected, found, .. }) => {
            assert!(message.contains("Expected mindmap header"));
...
fn test_icon_syntax_variations() {
  ...
  root
    ...
    assert_eq!(diagram2.root.children[0].icon, Some("fa fa-book".to_string()));
...
fn test_class_syntax_variations() {
  ...
  root
    ...
    assert_eq!(diagram3.root.children[0].class, Some("spaced-class".to_string()));
}
...
fn test_node_id_generation() {
  ...
  root
    ...
    fn collect_ids<'a>(node: &'a mermaid_parser::common::ast::MindmapNode, ids: &mut Vec<&'a String>) {
...
fn test_mixed_indentation_levels() {
  ...
  root
    ...
    assert_eq!(child2.children[0].children[0].text, "Great grandchild (extra indented)");
}
...
fn test_whitespace_preservation_in_text() {
  ...
  root
    ...
    assert_eq!(diagram.root.children[2].text, "Node   with   internal   spaces");
...
fn test_complex_real_world_mindmap() {
  ...
  root((Project Planning))
    ...
    assert_eq!(requirements.children[0].icon, Some("fa fa-list".to_string()));
