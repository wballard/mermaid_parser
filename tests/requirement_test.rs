...
use mermaid_parser::common::ast::{RequirementType, RiskLevel, VerificationMethod, RelationshipType};
use rstest::*;
...
fn test_requirement_files(#[files("test/requirement/*.mermaid")] path: PathBuf) {
    ...
    let first_line = clean_content.lines()
    ...
    if known_invalid_headers.iter().any(|&h| first_line.trim() == h) {
...
requirement "__test_req__" {
    ...
    // Test lexer
    let tokens = requirement::requirement_lexer()
        .parse(input)
        .into_result();
